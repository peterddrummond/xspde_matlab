...
%   n > 0 for x,k,o-space for computing output{n} and making comparisons
...
if n == 0                                        %%If n = 0, initial grids
  ...
  for c = 1:p.fieldcells                         %%loop over cells
    ...
    for c = 1:p.fieldcells                       %%loop over cells
      if p.points{c}(d) > 1 
        ...
        if isequal(bound,zeros(size(bound)))
          ...
          p.k{c,id} = reshape(p.k{c,id},[p.fields{c},p.d.space{c}]); 
        end
      end
    end                                          %%end loop over cells
  end                                            %%end loop on dimension
