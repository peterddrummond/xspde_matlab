...
\lyxaddress{\textbf{Centre for Quantum and Optical Science, Swinburne University
...
The xSPDE software toolbox is an e\textbf{x}tensible \textbf{S}tochastic
...
\emph{xSPDE is distributed without any guarantee, under the MIT open-source
...
To simulate a stochastic equation interactively, first make sure Matlab
path is pointing to the xSPDE folder. If not, proceed as follows:
\begin{itemize}
...
\hline 
\hline 
\emph{in.fields}  & integer  & $1$ & Stochastic field variables\tabularnewline
\hline 
\emph{in.noises}  & integer  & $1$ & Number of noises\tabularnewline
\hline 
...
\hline 
\emph{in.ensembles} & integer vector & {[}1,1,1{]} & Hierarchy of stochastic trajectories\tabularnewline
\hline 
\emph{in.observe}  & function & \emph{a} & Observable function \tabularnewline
\hline 
...
\hline 
\emph{in.images}  & integer cell array & \emph{\{0,..\}}  & Movie images in time per observable\tabularnewline
\hline 
\emph{in.transforms}  & vector cells  & \emph{ \{{[}0 0 0 0{]},..\}}  & Output Fourier transforms in {[}t,x,y,z{]}\tabularnewline
\hline 
\end{tabular}
\par\end{center}
...
\texttt{in.olabels = '<a\textasciicircum{}2>';}
...
\begin{itemize}
\item If you like mathematics; as an offline exercise, try to solve the
equivalent diffusion equation for the probability P to find the variance
$\left\langle a^{2}\right\rangle $! The equation is:
\[
...
steady-state fluctuations of any physical system. Take the random
walk equation in a complex space, 
\[
\frac{da}{dt}=-a+w(t)
\]
where $w(t)=w_{1}(t)+iw_{2}(t)$. Use an initial condition of $a=(v_{1}+iv_{2})/\sqrt{2}$,
with $\left\langle v_{i}^{2}\right\rangle =1$, to start near the
steady state. 
...
\]
The expectation value of the noise spectrum, in the large $T$ limit,
is therefore: 
\begin{eqnarray*}
...
\texttt{in.olabels = '|a(\textbackslash{}omega)|\textasciicircum{}2'; }
...
can transform any field in any dimension, with any number of resulting
...
The total dimension $d$ in xSPDE is unlimited, and includes time
and space. Large $d$ is memory-intensive and slow! The general equation
solved can be written in differential form as
...
initial conditions are arbitrary functions. $\mathbf{A}\left[\mathbf{a}\right]$
and $\underline{\mathbf{B}}\left[\mathbf{a}\right]$ are vector and
matrix functions of $\mathbf{a}$, $\underline{\mathbf{L}}\left[\bm{\nabla}\right]$
is a matrix of linear terms and derivatives, diagonal in the vector
indices, and $\mathbf{w}=\left[\bm{w}^{x},\bm{w}^{k}\right]$ are
real delta-correlated noise fields such that: 
\begin{eqnarray}
...
Transverse boundary conditions are assumed periodic. The term $\underline{\mathbf{L}}\left[\bm{\nabla}\right]$
...
ordinary space or momentum space: 
\begin{eqnarray*}
...
\texttt{in.olabels = '|a(x)|\textasciicircum{}2'; }
...
Here, the noise is delta-correlated, and $\Delta V$, $\Delta V_{k}$
...
and $V=R_{x}R_{y}$, where there are $N_{x(y)}$ points in the x(y)-direction,
with a total range of $R_{x(y)}$. Then, $\Delta x=R_{x}/N_{x}$ ,$\Delta k_{x}=2\pi/R_{x}$
, so that: 
\begin{eqnarray*}
...
\texttt{in.olabels= '<|a\_1(x)|\textasciicircum{}2>';}
...
Counter-intuitively, a random potential prevents normal wave-packet
...
function called $\mathbf{xspde}$. This calls \textbf{$\mathbf{xsim}$},
for the simulation, then $\mathbf{xgraph}$ for the graphics. Most
of the work is done by other specialized functions. Input parameters
come from an ${\bf input}$ array, output is saved either in a ${\bf data}$
...
field on the lattice, called \textbf{\emph{a.}} The default is $\mathbf{xinitialise}$,
which sets fields to zero. 
\item [{{*}observe}] is the initial observation function whose output is
averaged over the ensembles, called from \textbf{xpath}. The default,
$\mathbf{xobserve}$, returns the amplitudes. 
\item [{{*}function}] is an optional function used to take arbitrary transforms
or time integrals of the initial observe outputs, after averaging
over the first ensemble. The default returns the observe outputs unchanged. 
\item [{{*}linear}] is the linear response, including transverse derivatives
in space. The default, $\mathbf{xlinear}$, sets this to zero. 
\item [{{*}da}] is called by \textbf{step} to calculate derivatives at
every step in the process, including the stochastic terms. The default,
$\mathbf{xstep}$, sets this to zero
\item [{transfer}] is a function used to initialize sequential simulations,\textbf{
}where previous field values are included as input to the function.
The default, $\mathbf{xtransfer}$, replicates the output of the previous
simulation.
...
\section{Data arrays and errors}


\subsection{Data arrays}

All calculated data, including fields, observables and graphics results,
is stored in arrays of implicit or explicit rank $2+dimension$. The
first index is a field index ($i$), the second a statistics/errors
index ($j$), while the remaining indices $k\equiv k_{1},\ldots k_{D}\equiv k_{1},\bm{k}$
are for time and space. The space-time dimension $D$ is unlimited.
When the fields, noises or coordinates are integrated by the xSPDE
integration functions, they are flattened to a matrix. The first index
is the field index, and the combined second index covers all the rest. 

There are several different types of arrays used. These are as follows:
\begin{itemize}
\item Field arrays $a(i,j,1,\bm{k})$ - these have a statistics index of
up to $j=ensembles(1)$, but just a single present time-point for
efficiency.
\item Random and noise arrays $w(i,j,1,\bm{k})$ - these are like field
arrays, except that they contain random numbers for the stochastic
equations.
\item Coordinate arrays $x\{l\}(1,j,1,\bm{k})$- these store the values
of coordinates at grid-points, depending on the axis label $l=2,\ldots D$.
\item Raw arrays $r\{e,s\}(i,j,k)$ - like fields, but with all points stored.
Use with care, as they take up large amounts of memory. Note that
when output or saved, these have additional cell indices $e=1,2$
for the error-checking time-step and $s=1,\ldots S$ for the sequence
number.
\item Data arrays $d\{n\}(i,j,k)$ - these store the averages, or arbitrary
functions of them, with a statistics index $j=1,2,3$, to store error
data at all time points.
\item Graphics arrays $g\{n\}(i,j,k)$ - these store the data that is plotted,
and can include further functional transformations if required.
\end{itemize}
The field index $i$ in a graphics or data array is intended to describe
different lines on a graph. There can be quite different first dimensions
between fields, noises and output data, as they are specified using
different input parameters. If only a single output graph is wanted,
the $observe$ cell index is not needed.

Outputs have an extra high-level cell index $\{n\}$ called the graph
or function index. This corresponds to the index $\{n\}$ of the observe
function used to generate averages. One can have several data arrays
in a larger cell arrays to make a number of distinct output graphs
labelled $n$, each with multiple averages. 
...
error-bars if $in.errorchecks=2$ was specified, which is also the
default parameter setting. Error-bars below a minimum relative size
...
doing this. Error bars on the graphs can be removed by setting $errorchecks=1$
...
as follows:
\begin{description}
\item [{{*}function(o,r)}] is is a cell array of functions whose output
$\mathbf{xgraph}$ is designed to plot. There is one function for
each multidimensional dataset that is plotted. The default value is
just the average stored in the data array generated by the simulation
observe function, provided these are scalar observables. \\
\\
An arbitrary number of functions of these observables can also be
plotted, including vector observables. The input to functions is the
observed data averages, stored in a cell array of size $o\{ngraphs\}(points(1),nspace,ofields)$.
Here $ngraphs$ is the number of observable averaging functions, $points(1)$
is the number of time points,$nspace$ the number of space points,
$ofields$ the vector dimension of the averaging or observable function.
This can change depending on the observable.
...
The code is intended to cascade down from higher to lower dimension,
generating different types of user-defined graphs. Each type of graph
is generated once for each specified graphics function. The graphics
axes that are used for plotting, and the points plotted, are defined
using the optional \textbf{\emph{axes }}input parameters, where $axes\{n\}$
...
every 4-th x point, and al the y-points.
\end{itemize}
...
as this sets the maximum effective plotted dimension. For example,
$pdimension\{1\}=1$ means that only plots vs time are output for
the first function plotted. Note that in the following, $t,x,y,z$
may be replaced by corresponding higher dimensions if there are axes
...
Since the plot has to include a data axis, the plot will be typically
one dimension higher than the underlying space-time lattice: 
\begin{description}
...
Note that error bars and sampling errors are only graphed for 2D plots,
typically of results vs time. The error-bars are not plotted when
they are below a user-specified size, to improve graphics quality.
...
with '\emph{in.c}' or any capital letter like '\emph{in.}A...' are
always available to users of xSPDE. Note that use of globals is \textbf{not}
recommended, as it is incompatible with the Matlab parallel toolbox. 
...
\hline 
\hline 
\emph{version}  & \emph{'xSIM1.22'}  & Current version number\tabularnewline
\hline 
\emph{name}  & \emph{''}  & Simulation name\tabularnewline
\hline 
\emph{dimension}  & 1  & Space-time dimension\tabularnewline
\hline 
\emph{fields}  & 1  & Number of stochastic fields\tabularnewline
\hline 
...
\hline 
\emph{order}  & \emph{1}  & Extrapolation order \tabularnewline
\hline 
\emph{errorchecks}  & \emph{2}  & Number of error-checking cycles\tabularnewline
\hline 
...
\hline 
\emph{graphs}  & \emph{1}  & Number of observables\tabularnewline
\hline 
\emph{print}  & 1  & Print: 1 for medium, 2 for high output\tabularnewline
\hline 
...
\hline 
\emph{transforms}  & \emph{ \{{[}0 0 0 0{]},..\}}  & Fourier transforms in {[}t,x,y,z,..{]}\tabularnewline
\hline 
...
\hline 
\emph{{*}transfer}  & \emph{(v,r,a0,r0)} & Initialization in a sequence\tabularnewline
\hline 
...
\hline 
\emph{{*}function}  & \{@(av,\textasciitilde{})av\{1\}(:,:,:).\textasciicircum{}2,..\}  & Functions stored of averages\tabularnewline
\hline 
\emph{{*}compare}  & (t,in)  & Comparison function cell array\tabularnewline
\hline 
...
\hline 
\emph{xd} & (a,D,r) & Takes a spatial derivative\tabularnewline
\hline 
\end{tabular}
...
\hline 
\emph{$x,y,z$}  & array  & \emph{-}  & Grid of $x,y,z$\tabularnewline
\hline 
\emph{$kx,ky,kz$}  & array  & \emph{-}  & Grid of $k_{x},k_{y},k_{z}$\tabularnewline
\hline 
\emph{$Dx,Dy,Dz$}  & array  & \emph{-}  & Grid of $D_{x},D_{y},D_{z}$\tabularnewline
\hline 
\emph{$x\{1\},\ldots x\{d\}$}  & array  & \emph{-}  & Grid of\emph{$x_{1},\ldots x_{d}$} \tabularnewline
\hline 
\emph{$k\{1\},\ldots k\{d\}$}  & array  & \emph{-}  & Grid of\emph{ $k_{1},\ldots k_{d}$} \tabularnewline
\hline 
\emph{$D\{2\},\ldots D\{d\}$}  & array  & \emph{-}  & Grid of\emph{ $D_{2},\ldots D_{d}$} \tabularnewline
\hline 
...
\hline 
\emph{K}  & real  & \emph{1}  & Momentum lattice volume\tabularnewline
\hline 
\emph{dV}  & real  & \emph{1}  & Spatial cell volume\tabularnewline
\hline 
\emph{dK}  & real  & \emph{1}  & Momentum cell volume\tabularnewline
\hline 
...
\hline 
\emph{d.ft}  & vector  & \emph{{[}1 1 1{]}}  & Dimensions for field transforms\tabularnewline
\hline 
...
\hline 
\hline 
\emph{gversion}  & \emph{'xGRAPH1.22'}  & Current version number \tabularnewline
\hline 
...
\bibitem{xmds-source}http://sourceforge.net/projects/xmds/\end{thebibliography}
