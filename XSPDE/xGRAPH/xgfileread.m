...
if hflag == 0                                  %%if filename  Matlab type 
    ...
    try                                        %%test Matlab filename
        ...
        fprintf('xGRAPH warninig: file %s has no version name\n', fname);
    end
...
try                                            %%test HDF5 filename
    v = h5readatt(fname,'/','version');        %%get version name
...
%   Returns error message if file does not exist or invalid.
%   MIT license by P. D. Drummond & S. Kiesewetter (2015), see License
...
if hflag == -1
    ...
    error('xGRAPH error: file %s has invalid type\n', filename);
else
  if ~exist(filename, 'file')
    error('xGRAPH error: file %s doesn''t exist\n', filename);
  end
end
end

%Version 1.1  xreadname uses Matlab error message convention