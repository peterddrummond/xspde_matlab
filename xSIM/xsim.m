...
totfun = 0;                                      % initialize output count
for s = 1:seq                                    % loop over sequence
  for n = 1:input{s}.outputs                     % loop over outputs
 ...
 for n = 1:p.outputs                             % loop over outputs
  if p.d.data{n}(1) > 0                          % check if data present
    ...
    if p.relerr                                  % If relative errors                      
        ...
        rl = 'Relative';                         % relative normalisation
    else                                         % else absolute errors
        mx = 1;                                  % then don't scale
        rl = 'Absolute';                         % absolute normalisation
    end                                          % end if relative 
    if ne>1
    ...
    if p.bins{n} > 0                             % If probabilities needed
       ...
       d(:,sx) = sqrt(d(:,1)/p.scale{n});        % Poissonian sd estimate
    ...
    d(:,2) = sqrt(sum(d(:,2:p.nchk).^2,2));      % RMS summed step errors
    d(:,3) = d(:,sx);
    d      = d(:,1:3);
    esum = (d(:,3).^2+d(:,2).^2)+1.e-20;         % calculate error sum sq.
    ept = (esum > p.tol)&(abs(d(:,1))>p.cutoffs{n});% nonzero error points
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ~isequal(p.compare{n},[])                 % Check for comparisons
      ...
      dif = dif+zeros([g1points(1:(end-1)),1]);  % Pad zero to size
      [dif,~,~] = xcompress(n,dif,p);            % Compress data to axes
      ds = size(dif,gptlen);
      dif = reshape(dif,[p.d.data{n},1:ds]);     % get the comparisons
      d(:,4:3+ds) = dif; 
      ept = ept&(abs(d(:,4))>p.cutoffs{n});      % Get nonzero error points
      mx  = max(abs(d(:,4)).^2); 
      ...
      p.gpoints{n}(end) = 3+ds;                  % Increase size of points
      dif = (d(:,1)-d(:,4)).^2;                  % Get difference squared
      em(o,sx+1) = sqrt(max(dif)/mx);            % max mod dif scaled
      er(o,sx+1) = mean(dif)/mx;                 % mean dif squared, scaled
      if ne > 1  
        er(o,sx+2) = sum(ept.*(dif./esum));      % Chi squared
      end
      er(o,sx+3) = sum(ept);                     % Total points with errors
    end                                          % End check comparisons
    er(o,2:p.nchk) = er(o,2:p.nchk)/mx;
    p.axes{n}  = axes;                           % Compressed axes
    er(o,sx)  = mean(d(:,3).^2)/mx;              % Squared sampling errors
    ...
    for c = 2:p.nchk
    %d = p.checkd(c);                            % Dimension: future usage
    ...
    if ~isequal(p.compare{n},[])
      ...
      sumpts = mx*1.e-10+er(o,p.nchk+4);
      xpr(1,p,'Chi-square/points   = %.3g\n',er(o,sx+2)/sumpts);
    end
    xpr(1,p,'Normalised with max = %.3g\n',sqrt(mx));
