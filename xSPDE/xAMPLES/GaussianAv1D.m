function [e] = GaussianAv1D()
%   e  =  GAUSSIANAV1D() tests xSPDE for a linear Schroedinger equation
...
in.linear =      @(r) 1i*0.05*(r.Dx.^2);                  %%laplacian
in.transforms =  {0,0,[0,1]};                             %%transforms 
in.images =      {2};                                     %%number of images
in.transverse =  {2};                                     %%transverse plots
in.olabels =     {'I','<I>','\int I dV','I(k)'};          %%labels 
in.compare{1} =  @(t,~,~) (1+(t/10).^2).^(-1/2);               %%comparison
