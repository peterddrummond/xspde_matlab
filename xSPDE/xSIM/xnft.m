...
%   First and second field indices are not transformed.
...
if (length(sz) == 2)                         %%Standard matrix input
        a = reshape(a,[sz(1),r.d.int]);      %%Unflatten lattice
end
...
for nd = 2:maxind                            %% loop over space dimension 
    if trans(nd) >0                          %%if FFT required
        %a = fftshift(fft(a,[],1+nd)*r.kfact(nd),1+nd);%%Fourier transform
         a = fft(a,[],1+nd)*r.kfact(nd);     %%Fourier transform
    end                                      %%end if FFT required
end                                          %% end loop to max dimension 
a  = reshape(a,sz);                          %restore original dimension
end                                          %%end function