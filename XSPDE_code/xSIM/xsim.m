...
%   First dimension is the combined index, last dimension is the error
...
for s=1:sequence                                 %%loop over sequence
  ...
  input{s}.points(2:end)=input{s}.points(2:end)./input{s}.steps(2:end); 
...
for s=1:sequence                                 %%loop over sequence
  ...
  for n=1:r.functions
    ...
    data{s}{n}=reshape(data{s}{n},r.gpoints{n}); %%output: full indexing
    for nd=2:r.dimension
      if r.ftransforms{n}(nd)
         data{s}{n}=fftshift(data{s}{n},1+nd);
      end 
    end
  end
  if r.dimension > 1                                %%if transverse  
    r = r.grid(2,r);                                %%get graphics k-grid
...
if ~strcmp(r.file,'')                            %%if filename exists
    input = xwrite(input,data,raw);              %%save data in file
