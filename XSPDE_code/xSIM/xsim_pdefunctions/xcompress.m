function [data,x,axes]  =  xcompress(n,data,r) 
%   a = XCOMPRESS(n,data,r) compresses the data on the required axes. 
...
%   Axes: 'r.axes'; x,k-coordinates: 'r.xc','r.kc', switch: 'r.ftransforms' 
...
x =   r.xc;                                      %%initialize coordinates
m1 = length(x);                                  %%length of coord. list
if isfield(r,'oc')                               %%check if bin coordinates
    if n <= length(r.oc)                         %%check if n-th bin
        oc = r.oc{n};                            %%n-th bin coordinates
...
d = min([length(axes),length(x),length(np)-2]);  %%data axis dimensions
%
...
for nd = 1:d                                     %%Loop over dimension   
    ...
    if nd <= r.dimension && r.ftransforms{n}(nd) %%if transforms are set
        xnd =  r.kc{nd};                         %%graphics k coords     
