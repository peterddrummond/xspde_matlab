...
%   a  =  XBIN(o,[dx,] p) bins an input observable over a lattice. 
%   Input:  one component  variable 'o', bin size `dx', lattice 'r'.
%   Note 'dx' should have dimension 1 or at most equal to p.dimension
%   Output: '1' for coordinate inside bin, '0' otherwise.
%   Bins in first direction where dx(i) > 0 or uses dx as a scalap.
...
switch nargin                                %% checks for arguments
    ...
    case 2                                   %% case of two arguments
        ...
        dx = p.dx.*p.steps;                  %% dx is the volume element
...
if (length(sz) == 2)                         %% If standard matrix input
        o = reshape(o,[sz(1),p.d.int]);      %% Unflatten lattice
...
bin1  = p.r{nd}-dx/2;                        %% bin 1 is lower bin boundary
bin2  = p.r{nd}+dx/2;                        %% bin 2 is upper bin boundary
