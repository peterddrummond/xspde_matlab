...
for s = 1:sequence                               %%loop over sequence 
    ...
    input{s}.sized = sz;                         %%store data size
...
%% 
  ...
  for n = in.graphs                              %%Loop over graphs
    ...
    sized = in.sized{n};                         %%Get the graph data size
    if in.pdimension{n} > 0 && ~isempty(d)       %%Check if graph needed
      ...
      if in.errors == 1 && gp(end) > 2           %%check case of no errors
          gp = [gp,1];                           %#ok<AGROW> %%increment index
      ...
      for i = 1:len-2                            %%loop over data dimension
         ...
         if isempty(in.xk{n}{i})                 %%check if no coordinates 
           in.xk{n}{i} = 1:sized(1+i);              %%use data indices
...
%% 
      ...
      if gpe > 3                                 %%if comparisons exist
         ...
         if max(d(:,3).^2+d(:,6).^2) > 0         %%check if sampling error
           ...
           if in.print > 0                       %%if print switch
             fprintf('Chisq error total = %d\n',chs);%%chi-square error
             fprintf('Chisq points = %d \n',cn); %%print comparison point
           end                                   %%end if print switch
