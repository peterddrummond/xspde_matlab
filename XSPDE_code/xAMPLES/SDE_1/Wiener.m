...
%   xSPDE functions are licensed by Peter D. Drummond, (2015) - see License
 ...
 p.ensembles       =  [250,40];                    %%ensembles for averaging
 p.initial         =  @(v,~) 2*v;                  %%Initial function
 p.deriv           =  @(a,w,~) w;                  %%Derivative function
 p.raw             =  1;                           %%raw switch
 %p.file            =  'Wiener.h5';                %%may not work in Octave
 ...
 p.function{2}     =  @(o,~) o{1}(2,:);            %%Space-time function
 p.compare{2}      =  @(p) 4+p.t;                  %%Comparison handle
 p.function{3}     =  @(o,~) o{1}(4,:)  ;          %%Space-time function
