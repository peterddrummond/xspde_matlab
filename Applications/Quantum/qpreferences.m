...
p.theta   = xprefer(p,'theta',1,{0});            % default phase
nm        = min(p.modes,length(p.nmax));         % test length of nmax
p.nmax(1+nm:p.modes) = p.nmax(nm);               % set nmax on all modes
p.fields = prod(p.nmax);                         % wavefunction first dimension
if isscalar(p.L) && isempty(p.L{1})
  p.nl =[];
else
...
for n = p.nl                                     % loop over decay channels
  ...
  for m = 1:numel(g)                             % loop over decay rates
    ...
    if g(m) ~= 0.0 && p.quantum < 2              % nonzero decay   
...
if p.sparse                                      % check if sparse
    ...
    p.H = xprefer(p,'H',0,@(a,p) zeros([p.fields{1}(1),p.ensembles(1)]));
    p.L = xprefer(p,'L',0, {[]});
end  
